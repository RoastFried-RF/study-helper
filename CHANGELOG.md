...
### 수정
- **API 서버 정지 문제**: 변환(ffmpeg)·요약·텔레그램 전송·영상 다운로드 등 blocking 작업이 이벤트 루프를 멈춰, API 서버 모드에서 헬스체크·동시 요청이 함께 정지하던 문제 수정 (모든 blocking 호출을 워커 스레드로 위임)
- **자동 모드 진행 상태 손실**: 자동 모드와 유지보수 스크립트(`recover_missing`/`reconcile_progress`)를 동시 실행할 때 `auto_progress.json` 변경이 서로 덮어쓰이던 lost update 수정 — 저장 시 디스크 최신본을 다시 읽어 병합
- **Whisper OOM kill**: Docker `mem_limit` 안에서 모델이 cgroup 메모리 한도를 인식하지 못해 OOM kill 되던 문제 수정 — cgroup v1/v2 한도를 인식해 부족 시 더 작은 모델로 자동 다운그레이드
- **자동 모드 종료 hang**: `Ctrl+C` 종료 시 입력 감시 스레드가 프로세스 종료를 막던 문제 수정
- 백그라운드 재생 진도 보고의 페이지 번호가 0으로 계산되던 경계 오류 등 메모리·정확성 관련 다수 수정

### 추가
- **`WHISPER_CPU_THREADS`** 환경변수 — STT(faster-whisper) CPU 스레드 상한 (기본 2). 미설정 시 STT가 호스트 전 CPU를 점유하던 동작 완화
- **`PROGRESS_SAVE_INTERVAL`** 환경변수 — 자동 모드 진행 상태 저장 주기 (기본 5강의마다)

### 변경
- **긴 강의 AI 요약**: 장시간 강의의 STT 텍스트를 청크로 분할 요약 후 통합 — 모델 입력 토큰 한도 초과 방지
- **자동 모드 진행 저장 배치화**: 강의마다 전체 직렬화하던 것을 주기적 저장으로 변경 — 디스크 쓰기 횟수 감소
